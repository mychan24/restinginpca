---
title: "DiSTATIS on edges derived from Gordon Group parcellation (n nodes = 333)"
...
```
```{r plot_fig_netf, echo=F, fig.height=8, fig.width= 16}
gridExtra::grid.arrange(as.grob(f01.netF), as.grob(f01.netF.cp3), ncol=2)
```
...
# f02.FpF.sub
f03.FpF.sess.cp3 <- f.graph4sess.cp3$zeMap_background + pf.sess.cp3$pointsColByBlocksColByBlocks + pf.sess.cp3$linesColByBlocks + f.graph4sess.cp3$zeMap_dots + f.labels.23
# f02.FpF.sub
# f02.FpF.sess
```
...
save.image(file = "cfDiSTATIS_GordonParcel.RData")
```
