---
title: "DiSTATIS on mean connectivity of edges derived from indivisual parcellation + simmulations"
...
session.color <- list(oc = as.matrix(rep(c("red","blue"),20)),
                       gc = as.matrix(rep(c("red","blue"),2)))
...
# f02.FpF.sub
f02.FpF.sess <- f.graph4sess$zeMap_background + pf.sess$pointsColByBlocks + pf.sess$linesColByBlocks + f.graph4sess$zeMap_dots + f.labels
# f02.FpF.sub
# f02.FpF.sess
```
...
# f02.FpF.sub
f03.FpF.sess.cp3 <- f.graph4sess.cp3$zeMap_background + pf.sess.cp3$pointsColByBlocks + pf.sess.cp3$linesColByBlocks + f.graph4sess.cp3$zeMap_dots + f.labels.23
# f02.FpF.sub
# f02.FpF.sess
```
...
save.image(file = "cfDiSTATIS_simattack_IndivParcel_EdgeMeans.RData")
```
