...
suppressMessages(library(PTCA4CATA)) # devtools::install_github("HerveAbdi/PTCA4CATA")
suppressMessages(library(ExPosition))
suppressMessages(library(InPosition))
suppressMessages(library(MExPosition))
suppressMessages(library(superheat)) # devtools::install_github("mychan24/superheat")
...
load(paste0("../../../data/grandatble_and_labels_MSC01_MSC02_MSC08_20190410.Rdata")) # read the labels of grandtable
# gt: grand tabls
...
```{r plot_all_sub_session_hmap, echo=F, fig.height=(7*length(subj.name)), fig.width= 40}
...
# == Get singular value of net work edges == #
# for partial factor score?
subtab_i <-  gtlabel$subjects_edge_label      # Define network edges mini-table
n_subtable <- length(unique(subtab_i))  # Number of subject x network edges mini tables

sv <- matrix(NA, 1, n_subtable)     # Initialize eigenvalues object
colnames(sv) <- levels(gtlabel$subjects_edge_label)
...
## Set heat limit
quantlim <- quantile(unlist(fj1_sqmat), c(.25,.75))
heatlim <- c(-round(min(min(abs(quantlim))), digits=3), # limit equal on pos/neg, round 3digits
             round(min(min(abs(quantlim))), digits=3))
