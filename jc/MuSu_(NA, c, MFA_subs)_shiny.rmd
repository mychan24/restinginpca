...
inputPanel(
  ...
  awesomeCheckboxGroup(inputId = "sub2plot",
                       ...
                       selected = c(),inline = TRUE, status = "success")
...
inputPanel(  
  ...
  awesomeCheckboxGroup(inputId = "f_edgeOI", 
                       label = "Edges to show:", choices = unique(net.edge),
                       selected = c(),inline = TRUE, status = "warning"),
...
Checkplot_f_edge <- eventReactive(input$plot_edge,{
  ...
  mean.fj2plot <- mean.fj[which(net.edge %in% input$f_edgeOI),] 
  ...
  col.max10 <- col4ImportantEdg[which(net.edge %in% input$f_edgeOI)] %>% .[fj2plot.ind]
