...
# Procedure outline:-------------------------------
#--1. Correlation matrix (subj1, sessions 1-10) >> rectangular matrix (sessions x voxels by network)
#   >> center across sessions >> SVD
#--2. Correlation matirx (subj1, sessions 1-5; 6-10) >> rectangular matrix [sessions x voxels by network by subject] 
##  >> center across sessions >> SVD
...
#--- check centered data in square matrix
zmat_centXsess <- sapply(1:nrow(gt.sub1_centXsess), function(x) vec2sqmat(gt.sub1_centXsess[x,]), simplify = "array")
img <- image_graph(800, 1000)
out <- lapply(1:10, function(session.count){
  hmap <- superheat(zmat_centXsess[,,session.count],
