---
title: "MuSu_(NA, c, MFA_subs) - MSC #1 Subjects (N=1)"
...
# Load Grand Table
# gt: grand tabls
# gtlabel: information for each edge in the rectangular grand table
load(paste0("../data/grandatble_and_labels_MSC_subs1_20190602.Rdata")) # read the gtlabel of grandtable
...
#--------------
subtab_i <-  list(gtlabel$subjects_label)    # Use subjects_edge_label as mini table
...
cjplot_imp <- createFactorMap(c_edge[importantEdg,],
                              ...
                              col.gtlabel = col4ImportantEdg[importantEdg],
                              text.cex = 2,
                              force = 0.5,
                              title = "Significant contribution")

f.gtlabel <- createxyLabels.gen(x_axis = 1, y_axis = 2,
                               lambda = svd.res$ExPosition.Data$eigs,
                               tau = svd.res$ExPosition.Data$t,
                               axisName = "Dimension ")

ciplot01 <- ciplot$zeMap + f.gtlabel
cjplot01 <- cjplot_all$zeMap_background + cjplot_all$zeMap_dots + f.gtlabel
cjplot02 <- cjplot_imp$zeMap_background + cjplot_imp$zeMap_dots + cjplot_imp$zeMap_text + f.gtlabel
```
...
f_netedge_plot01 <- plot.f_edge$zeMap_background + plot.f_edge$zeMap_dots + f.gtlabel
# plot the significantly contributed factor scores
plot.f_edge_imp <- createFactorMap(mean.fj, axis1 = 1, axis2 = 2,
                                   col.points = col4ImportantEdg,
                                   col.gtlabel = col4ImportantEdg,
                                   text.cex = 2,
                                   force = 0.5,
                                   title = "Significantly contributed factor scores - colored by subject_x_edge types")
f_netedge_plot02 <- plot.f_edge_imp$zeMap_background + plot.f_edge_imp$zeMap_dots + f.gtlabel
...
f_netedge_plot03 <- plot.f_impedge$zeMap + f.gtlabel
...
plot.f_bw_imp <- createFactorMap(mean.fj.bw, axis1 = 1, axis2 = 2,
                                 ...
                                 col.gtlabel = net.edge.col$gc[rownames(mean.fj.bw),],
                                 text.cex = 3,
                                 force = 0.5,
                                 title = "Mean factor scores for subject_x_edge types")
f_netedge_plot04 <- plot.f_bw_imp$zeMap_background + plot.f_bw_imp$zeMap_dots + plot.f_bw_imp$zeMap_text + f.gtlabel
...
f_impedge.CI <- MakeCIEllipses(getBoot2plot_edge,
                               ...
                               p.level = .95)
# create plot
f_CInetedge_plot01 <- plot.f_impedge$zeMap_background + plot.f_impedge$zeMap_dots + f.gtlabel
f_CInetedge_plot02 <- plot.f_impedge$zeMap_background + plot.f_impedge$zeMap_dots + f_impedge.CI + f.gtlabel
...
plot.CI.f_bw <- MakeCIEllipses(getBoot2plot_edge_bw,
                               ...
                               p.level = .95)
# create plot
f_CInetedge_plot03 <- plot.f_bw_imp$zeMap + plot.CI.f_bw + f.gtlabel
```
...
# fj.rot12 <- varimax(svd.res$ExPosition.Data$fj[,c(1,2)], normalize = FALSE)$loadings
# for(i in 1:length(subj.name)){
#   fj1_sqmat[[i]] <- vec2sqmat(fj.rot12[,1]) 
#   fj2_sqmat[[i]] <- vec2sqmat(fj.rot12[,2])   
# }
...
To compare results from MuSu to DiSTATIS, we try to run MDS with factor scores. Take the first two component for example, we run MDS on each component:
...
```{r screeplot1, echo = FALSE}
### eigen values
PlotScree(ev = MDSf1$ExPosition.Data$t,
          title = "Expained variance per dimension")

### MDS factor scores for nodes
f1.MDSgraph <- createFactorMap(MDSf1$ExPosition.Data$fi,
                           axis1 = 1, axis2 = 2,
                           title = "Compromise - Factor scores (nodes)",
                           col.points = parcel.list$sub01$Comm.col$oc,
                           alpha.points = .4, cex = 4)
f1.MDSlabels <- createxyLabels.gen(x_axis = 2, y_axis = 3,
                               lambda = MDSf1$ExPosition.Data$eigs,
                               tau = MDSf1$ExPosition.Data$t,
                               axisName = "Dimension ")
### Show plot
MDSf1.noLabel <- f1.MDSgraph$zeMap_background + f1.MDSgraph$zeMap_dots + f1.MDSlabels
print(MDSf1.noLabel)
```
...
### eigen values
PlotScree(ev = MDSf2$ExPosition.Data$t,
          title = "Expained variance per dimension")

x_axis = 2
y_axis = 7

### MDS factor scores of nodes
f2.MDSgraph <- createFactorMap(MDSf2$ExPosition.Data$fi,
                           axis1 = 1, axis2 = 2,
                           title = "Compromise - Factor scores (nodes)",
                           col.points = parcel.list$sub01$Comm.col$oc,
                           alpha.points = .4, cex = 4)
f2.MDSlabels <- createxyLabels.gen(x_axis = x_axis, y_axis = y_axis,
                               lambda = MDSf2$ExPosition.Data$eigs,
                               tau = MDSf2$ExPosition.Data$t,
                               axisName = "Dimension ")
### Show plot
MDSf2.noLabel <- f2.MDSgraph$zeMap_background + f2.MDSgraph$zeMap_dots + f2.MDSlabels
print(MDSf2.noLabel)
