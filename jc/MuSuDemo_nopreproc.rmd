...
## Objectives
Here, we illustrate the results when the rectangular matrix (sessions x voxels by network by subject) is analyzed by SVD without any centering and normalizing
...
The data is transposed so that the rows are now different edges of different subject and the columns are different sessions.
