...
## Plot
```{r plotsetup, echo=F}
#--- text color of text label (background color matches systems)
textcol <- c(rep("black",8),rep("white",2),rep("black",3),"white",rep("black",3),rep("white",2))
...
labelcol <- Comm.col$gc[order(rownames(Comm.col$gc))]
...
heats1a <- superheat(fi1_s1a_sqmat, y.axis.reverse = T,
          ...
          left.label.col=labelcol,
          bottom.label.col=labelcol,
          ...
          left.label.text.size = 3,
          bottom.label.text.size = 2,
          left.label.text.col = textcol, 
          bottom.label.text.col = textcol, 
          title="sub01_a")
...
heat_sigfi_sub1a <- superheat(fi_sub1a_sig_sqmat, y.axis.reverse = T,
                              membership.rows =vox.des$Comm.rcd,
                              membership.cols =vox.des$Comm.rcd,                    
                              left.label.col= labelcol,
                              bottom.label.col= labelcol,
...
heat_sigfi_sub1b <- superheat(fi_sub1b_sig_sqmat, y.axis.reverse = T,
                              membership.rows =vox.des$Comm.rcd,
                              membership.cols =vox.des$Comm.rcd,                    
                              left.label.col= labelcol,
                              bottom.label.col= labelcol,
...
gridExtra::grid.arrange(as.grob(heat_sigfi_sub1a$plot), as.grob(heat_sigfi_sub1b$plot), ncol=2,
                        top = textGrob("Node x Node Matrix of Factor Score w/ Sig Contribution",gp=gpar(fontsize=16,font=3)))
```

Smoothed Sig Factor Score
```{r, sm_squaremat_sigfi, echo=F, fig.show = 'hide'}
#--- the important variables are the ones that contribute more than or equal to the average
sm_heat_sigfi_sub1a <- superheat(fi_sub1a_sig_sqmat, y.axis.reverse = T, 
...
sm_heat_sigfi_sub1b <- superheat(fi_sub1b_sig_sqmat, y.axis.reverse = T, 
                                  smooth.heat = T, smooth.heat.type = "mean",
                                  membership.rows =vox.des$Comm.rcd,
                                  membership.cols =vox.des$Comm.rcd,                    
                                  left.label.col= labelcol,
...
gridExtra::grid.arrange(as.grob(sm_heat_sigfi_sub1a$plot), as.grob(sm_heat_sigfi_sub1b$plot), ncol=2,
                        top = textGrob("Smoothed Node x Node Matrix of Factor Score w/ Sig Contribution",gp=gpar(fontsize=16,font=3)))
```
