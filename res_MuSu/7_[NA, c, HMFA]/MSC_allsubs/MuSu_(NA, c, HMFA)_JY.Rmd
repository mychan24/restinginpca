...
# select color for subjects (subtables)
plot.pf_sess <- createPartialFactorScoresMap(
  factorScores = svd.res$ExPosition.Data$fi[c(2,6),],
  partialFactorScores = pFi[c(2,6),,],
  axis1 = 1, axis2 = 2,
  names4Partial = subj.name,
  font.labels = "bold"
)
...
plot.f_edge_imp <- createFactorMap(mean.fj[which(net.edge %in% edgeOI),], axis1 = 1, axis2 = 2,
                                   col.points = col4ImportantEdg[which(net.edge %in% edgeOI)],
                                   col.labels = col4ImportantEdg[which(net.edge %in% edgeOI)],
                                   text.cex = 2,
                                   force = 0.5,
