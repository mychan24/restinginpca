...
---
title: "MuSu_(NA, c, HMFA) - MSC All Subjects (N=10)"
...
load(paste0("../../../data/grandatble_and_labels_MSC_allsubs_N10_20190419.Rdata")) # read the labels of grandtable
...
#============try to show subject by subject=================
# plot by subject
plot.pf_sess <- createPartialFactorScoresMap(
  factorScores = svd.res$ExPosition.Data$fi,
  partialFactorScores = pFi[,,c(1,7)],
  axis1 = 1, axis2 = 2,
  names4Partial = subj.name[c(1,7)],
  font.labels = "bold"
)

# plot by session
plot.pf_sess <- createPartialFactorScoresMap(
  factorScores = svd.res$ExPosition.Data$fi[c(1,7),],
  partialFactorScores = pFi[c(1,7),,],
  axis1 = 1, axis2 = 2,
  names4Partial = subj.name,
  font.labels = "bold"
...
plot.f_edge <- createFactorMap(mean.fj[which(BootRatio.Comm12 == TRUE),], axis1 = 1, axis2 = 2,
                               col.points = net.edge.col$oc[which(BootRatio.Comm12 == TRUE)],
                               text.cex = 2,
                               force = 0.5,
...
plot.f_edge_imp <- createFactorMap(mean.fj[which(BootRatio.Comm12 == TRUE),], axis1 = 1, axis2 = 2,
                                   col.points = col4ImportantEdg[which(BootRatio.Comm12 == TRUE)],
                                   col.labels = col4ImportantEdg[which(BootRatio.Comm12 == TRUE)],
                                   text.cex = 2,
                                   force = 0.5,
...
#===============try to show edge by edge====================================
edgeOI <- c("sub01_Within","sub02_Within","sub03_Within","sub04_Within","sub05_Within","sub06_Within","sub07_Within","subj08_Within")

plot.f_edge_imp <- createFactorMap(mean.fj[which(importantEdg == TRUE & net.edge %in% edgeOI),], axis1 = 1, axis2 = 2,
...
```{r, grid_smheat_sigfj2, echo=F, ig.height=20, fig.width=40}
