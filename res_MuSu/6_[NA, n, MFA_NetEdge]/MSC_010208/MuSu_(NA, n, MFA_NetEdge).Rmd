...
---
title: "MuSu_(NA, n, MFA_NetEdge) - MSC01, MSC02, MSC08 (bad sub)"
...
load(paste0("../../../data/grandatble_and_labels_MSC01_MSC02_MSC08_20190411.Rdata")) # read the labels of grandtable
# gt: grand tabls
...
# Center Columns (across rows) + MFA normalize mini-block (Edge Type)
cgt <- expo.scale(gt, center = T, scale = "ss1")
cgt[is.nan(cgt)] <- 0   # replace NaN with zero 

# == Get singular value of subjects == #
# for partial factor score?
subtab_i <-  gtlabel$subjects_label     # Define subjects mini-table
n_subtable <- length(unique(subtab_i))  # Number of subject mini tables

sv_sub <- matrix(NA, 1, n_subtable)     # Initialize eigenvalues object
colnames(sv_sub) <- levels(gtlabel$subjects_label)
for(i in 1:n_subtable){
  sv_sub[i] <- (svd(cgt[,subtab_i==unique(subtab_i)[i]])$d[1]) # singular value from svd 
}

# == MFA-preproc with edges_label == #
edgetab_i <- gtlabel$edges_label        # Define edge-type mini-table
n_edgetable <- length(unique(edgetab_i))  # Number of edge-type mini tables
sv_edge <- matrix(NA, 1, n_edgetable) 
for(i in 1:n_edgetable){
  sv_edge[i] <- (svd(cgt[,edgetab_i==unique(edgetab_i)[i]])$d[1]) # singular value from svd 
}

mfa_preproc <- cgt
for(i in 1:n_edgetable){
   mfa_preproc[,edgetab_i==unique(edgetab_i)[i]] <- mfa_preproc[,edgetab_i==unique(edgetab_i)[i]]/sv_edge[i]
}

# set the column names
gt_preproc <- mfa_preproc
gt_preproc[is.nan(gt_preproc)] <- 0   # replace NaN with zero 
gt_preproc[is.na(gt_preproc)] <- 0   # replace NA with zero * MFA prepro of edge-type minitable produces NA and not NaN - MC 20190411
colnames(gt_preproc) <- gtlabel$subjects_edge_label
dim(gt_preproc) # check dimension

# show the weight for each subject
1/sv_sub
```
...
```{r grid_heat_fi, echo=F, fig.height=8, fig.width=(7*length(subj.name))}
print("Dimension 1")
