---
title: "DoubleCent_(NA, NA, MFA_subs) - MSC All Subjects (N=10)"
...
load("../../data/grandatble_and_labels_MSC_allsubs_DC_20190619.Rdata") # read the labels of grandtable
...
cgt <- expo.scale(gt, center = F, scale = F)
...
Fi.constraints <- minmaxHelper(svd.res$ExPosition.Data$fi)

new.constraints <- list(minx = min(pF.constraints$minx,Fi.constraints$minx),
                        miny = min(pF.constraints$miny,Fi.constraints$miny),
                        maxx = max(pF.constraints$maxx,Fi.constraints$maxx),
                        maxy = max(pF.constraints$maxy,Fi.constraints$maxy))


# plot again the factor map
plot.f_sess4pF <- createFactorMap(svd.res$ExPosition.Data$fi,
                                  constraints = new.constraints) # with t(gt)
