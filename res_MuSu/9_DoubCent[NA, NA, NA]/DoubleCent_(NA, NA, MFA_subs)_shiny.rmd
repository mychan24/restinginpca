---
title: "DoubleCent_(NA, NA, MFA_subs)_shiny"
author: "Ju-Chi.Yu"
date: "6/20/2019"
...
inputPanel(
  ...
  awesomeCheckboxGroup(inputId = "dimension2plot",
                       label = "Pick the dimenisons:", choices = c(1:10),
                       selected = c(1,2),inline = TRUE, status = "warning")
  
)

Checkplot_pF <- eventReactive(c(input$plot_ses),{
  plot.pf <- createPartialFactorScoresMap(
    ...
    axis1 = as.numeric(input$dimension2plot[1]), axis2 = as.numeric(input$dimension2plot[2]),
    size.points = 3,
    size.labels = 3,
...
Checkplot_f_edge <- eventReactive(input$plot_edge,{
  ...
  dist2c <- sqrt(fj2plot.sq[,as.numeric(input$dimension2plot[1])]+fj2plot.sq[,as.numeric(input$dimension2plot[2])]) # take the sqrt of the ss of component 1+2
  ...
  # plot the image
  plot.f_edge_imp <- createFactorMap(mean.fj2plot[fj2plot.ind,as.numeric(input$dimension2plot)], 
                                     axis1 = 1, 
                                     axis2 = 2,
...
inputPanel(
  ...
  awesomeCheckboxGroup(inputId = "edgeOI",
                       label = "Edges to show:", choices = unique(net.edge),
                       selected = c(),inline = TRUE, status = "danger")
)
