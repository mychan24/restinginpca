...
gridExtra::grid.arrange(as.grob(p_dc), as.grob(p_e), ncol=2)
```
...
```{r o_n_mat, echo=F, fig.width=14}
gridExtra::grid.arrange(as.grob(p_o), as.grob(p_n), nrow=1)
```
...
```{r NormSS_endX,echo = F,  fig.show='hide'}
...
```{r mean_endX, echo = F, fig.show='hide'}
...
```{r grid_small_mats, echo=F, fig.width=14}
gridExtra::grid.arrange(as.grob(p_os2), as.grob(p_ons2), as.grob(p_om),
                        as.grob(p_ns2), as.grob(p_nns2), as.grob(p_nm), 
                        ncol=3)
```
...
print(corX.p23$zeMap + labels4corX23)
```
