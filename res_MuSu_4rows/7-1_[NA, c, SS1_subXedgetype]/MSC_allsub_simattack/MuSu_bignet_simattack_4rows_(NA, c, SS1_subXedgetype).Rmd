---
title: "MuSu_(NA, c, SS1_subXedgetype) - MSC All Subjects (N=10) Big Networks 4 Sessions (rows)- Sim Attack"
...
+ Normalizing: Normalized each subjXedgetype sub-table to ss = 1 (e.g., between-sub01, within-sub01, ...)

First we compute the weights that are used to normalized each subject table. These weights are computed as the inverse of the sums of squares:
...
for (k in 1:n_lv){
  ...
  subtab.weight <- as.vector(matrix(NA, n_subtable[[k]]))
  names(subtab.weight) <- unique(subtab_i[[k]])
  
  for(i in 1:n_subtable[[k]]){
    target.mat <- cgt_preproc[,subtab_i[[k]]==unique(subtab_i[[k]])[i]]
    # sqare root of SS of the subtable as weights
    subtab.weight[unique(subtab_i[[k]])[i]] <- sqrt(sum(apply(target.mat,2,"^",2)))
    # normalize subtables to ss = 1
    cgt_preproc[,subtab_i[[k]]==unique(subtab_i[[k]])[i]] <- cgt_preproc[,subtab_i[[k]]==unique(subtab_i[[k]])[i]]/subtab.weight[i]
  }
}

# set the column names
gt_preproc <- cgt_preproc
...
```{r svd, include = FALSE}
svd.res <- epPCA(cgt_preproc,scale = FALSE, center = FALSE, graphs = FALSE)
```
...
save.image(file = "MuSu_simattack_4rows_7_ss1_subXedgetype.RData")
```
