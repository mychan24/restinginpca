---
title: "MuSu_(NA, c, MFA_subs) - MSC All Subjects (N=10) Big Networks 4 rows- Sim Attack"
...
* In all *subjects* and Even-numbered *sessions* (2 & 4), Reduce within connectivity in Default Mode (Syslabel: 1), Default-FP (1_3) and Default_VAN (1_5).
    + identify edges within default connectivity, default-FP and default-VAN
...
gt_preproc <- mfa_preproc                 # set the column names
...
#lapply(1:n_lv, function(x){1/sv[[x]]})    # show the weights for each table
```
...
save.image(file = "MuSu_simattack_4rows_3_MFA_subs.RData")
```
